...
- (void)paste:(id)sender {
    if (enableMentionsPluginV2) {
        ...
        if ([self.stringCopiedFromCurrentTextView length] > 0 && implementsWillCustomPasteTextInRange) {
            ...
            if(copyString != nil)
            {
                NSMutableAttributedString *nonNullable = [copyString mutableCopy];
                [string replaceCharactersInRange:selectionRangeBeforePaste withAttributedString: nonNullable];
            }
